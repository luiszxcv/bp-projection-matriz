// apply_wtp_patched_notier.cjs
// Patched WTP application aligning with `wtp meses.md` without tier multipliers
  ...
  for (let monthIndex = 0; monthIndex < months.length; monthIndex++) {
    ...
    for (const tier of TIERS) {
      ...
      for (const cohort of wtpCohorts[tier]) {
        ...
        const targetRaw = cohort.totalShareOfWallet * desiredPercent;
