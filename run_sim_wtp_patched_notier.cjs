...
try {
  ...
  const calc = require(path.join(repoRoot, "frontend", "temp_test", "calculations.js"));
  const inputs = require(path.join(repoRoot, "frontend", "temp_test", "defaultInputs.cjs"));
  const wtp = require(path.join(repoRoot, "frontend", "temp_test", "apply_wtp_patched_notier.cjs"));
  ...
  const summary = monthsWithWTP.map((m) => {
    return {
      ...
      wtpByTier: tiers.map((t) => ({
        tier: t,
        revenueExpansion: (m.wtpData && m.wtpData[t] && m.wtpData[t].revenueExpansion) || 0,
      })),
    };
  });
